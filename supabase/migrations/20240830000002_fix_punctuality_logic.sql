-- Fix punctuality score logic to not give perfect scores for zero attendance
-- This migration fixes the issue where employees with no attendance get 100% punctuality
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE 0  -- Changed from 100 to 0 - no attendance means no punctuality data
    ...
    CASE 
      ...
      ELSE 0  -- Changed from 100 to 0 - no leave requests means no leave planning data
    ...
    CASE 
      ...
      ELSE 0  -- Changed from 50 to 0 - no conversations means no communication data
    END::NUMERIC as communication_score,
    
    -- Overall HR Score (weighted average with data availability consideration)
    ROUND(
      CASE 
        WHEN (COALESCE(tm.total_tasks, 0) + COALESCE(am.days_present, 0) + COALESCE(cm.total_conversations, 0)) = 0 
        THEN 0  -- No data available
        ELSE 
          -- Calculate weighted average only for available data
          (
            CASE WHEN COALESCE(tm.total_tasks, 0) > 0 THEN COALESCE(tm.task_completion_rate, 0) * 0.3 ELSE 0 END +
            CASE WHEN working_days > 0 THEN (COALESCE(am.days_present, 0) * 100.0 / working_days) * 0.25 ELSE 0 END +
            CASE WHEN COALESCE(am.days_present, 0) > 0 THEN GREATEST(0, 100 - ((COALESCE(am.late_arrivals, 0) + COALESCE(am.early_departures, 0)) * 50.0 / am.days_present)) * 0.25 ELSE 0 END +
            CASE WHEN COALESCE(lm.total_leave_requests, 0) > 0 THEN GREATEST(0, 100 - ((COALESCE(lm.post_facto_requests, 0) * 100.0) / lm.total_leave_requests)) * 0.1 ELSE 0 END +
            CASE WHEN COALESCE(cm.total_conversations, 0) > 0 THEN (COALESCE(cm.avg_conversation_quality, 0) * 20) * 0.1 ELSE 0 END
          ) / (
            -- Adjust denominator based on available data
            CASE WHEN COALESCE(tm.total_tasks, 0) > 0 THEN 0.3 ELSE 0 END +
            CASE WHEN working_days > 0 AND COALESCE(am.days_present, 0) > 0 THEN 0.5 ELSE 0 END +  -- 0.25 + 0.25 for attendance and punctuality
            CASE WHEN COALESCE(lm.total_leave_requests, 0) > 0 THEN 0.1 ELSE 0 END +
            CASE WHEN COALESCE(cm.total_conversations, 0) > 0 THEN 0.1 ELSE 0 END +
