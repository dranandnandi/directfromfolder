...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'task_messages' AND indexname = 'idx_task_messages_task_id_v3'
  ) THEN
    CREATE INDEX idx_task_messages_task_id_v3 ON task_messages(task_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'task_activity_logs' AND indexname = 'idx_task_activity_logs_task_id_v3'
  ) THEN
    CREATE INDEX idx_task_activity_logs_task_id_v3 ON task_activity_logs(task_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'task_messages' AND indexname = 'idx_task_messages_user_id_v3'
  ) THEN
    CREATE INDEX idx_task_messages_user_id_v3 ON task_messages(user_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'task_activity_logs' AND indexname = 'idx_task_activity_logs_user_id_v3'
  ) THEN
    CREATE INDEX idx_task_activity_logs_user_id_v3 ON task_activity_logs(user_id);
  END IF;
END $$;
...
BEGIN
  ...
  CREATE POLICY "task_messages_select_policy_v3" 
  ...
  CREATE POLICY "task_messages_insert_policy_v3"
  ...
  CREATE POLICY "task_activity_logs_select_policy_v3"
